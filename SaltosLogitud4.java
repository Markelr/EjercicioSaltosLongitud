    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < saltos.length; i++) {
            ...
            for (int j = 0; j < saltos[i].length; j++) {
                if (saltos[i][j] == 0) {
                    saltosNulos++;
                } else if (mejorSalto == -1 || saltos[i][j] > mejorSalto) {
                    mejorSalto = saltos[i][j];
                }
            }

            System.out.println("Atleta " + (i + 1) + ":");
            System.out.println("  Cantidad de saltos nulos: " + saltosNulos);
            System.out.println("  Mejor salto: " + mejorSalto == -1 + "No hay saltos válidos" + mejorSalto);
        }
    }
}